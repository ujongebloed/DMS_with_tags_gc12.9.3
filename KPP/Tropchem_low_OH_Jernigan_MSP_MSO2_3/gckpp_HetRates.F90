...
!
      ...
      SPC_SO4  = spcVec(H%SO4%mId   ) + spcVec(H%SO4b%mId    ) 
      ...
      HSO3conc_Cld = State_Chm%HSO3_AQ(I,J,L) + State_Chm%HSO3_AQb(I,J,L)

      ! Cloud sulfite (SO3--) concentration [mol/l] from sulfate_mod.F
      SO3conc_Cld  = State_Chm%SO3_AQ(I,J,L) + State_Chm%SO3_AQb(I,J,L)
      ...
      !========================================================================
      HET(ind_HPMTF,  1) = HetTropAer( H%HPMTF%MW_g,  1.6E-3_fp )
      HET(ind_HPMTF,  2) = CloudHet( CldFr, Aliq, Aice,                      &
                                     rLiq,  rIce, TempK,   XDenA,            &
                                     H%HPMTF%MW_g,  1E-2_fp, 1E-2_fp)
...
!         ENDIF
        ...
        kITemp = kITemp                                                       &
               + HETMSA( TempK,        xRadi(12),  xArea(12),                 &
                         msaConc,      CONC_OH,                               &
                         XDenA,        XVOL(12),     5.0_fp,      H , 1      )

        ! Cloud k
        kITemp = kITemp                                                       &
               + HETMSA( TempK,        rLiq,       aLiq,                      &
                         msaConc,      CONC_OH,                               &
                         XDenA,        XVOL(12),   pHCloud,     H ,  0     )

        kITemp = kITemp                                                       &
               + HETMSA( TempK,        rIce,       aIce,                      &
                         msaConc,      CONC_OH,                               &
                         XDenA,        XVOL(12),   4.5_fp,      H ,  0    )
...
!
      ...
      REAL(fp),       INTENT(IN) :: C_OH        ! O3 in [mol/l]
      REAL(fp),       INTENT(IN) :: denAir      ! Density of air (#/cm3)i
      REAL(fp),       INTENT(IN) :: xvol        ! volume of aerosol per volume air
      REAL(fp),       INTENT(IN) :: pH
      INTEGER,        INTENT(IN) :: is_Aer      ! 1 = is aerosol, 0 = is cloud
      TYPE(HetState), POINTER    :: H           ! Hetchem species metadata
!
! !RETURN VALUE:
!
...
!
      ...
      ELSEIF (is_Aer == 1 ) THEN
      GAM = Gamma_MSA_AER (Radius, denAir,xvol,TK, cavg, C_MSA_g, C_OH, pH, H)
      ENDIF
...
!
      ...
      k_b      = 4.72e11_fp * dexp(-1270_fp / T)        !M-1 s-1  (Zhu et al. 2006)
...
! !INTERFACE:
!
    FUNCTION GAMMA_MSA_AER( Radius, n_air, xvol, T, cavg, C_MSA_g, C_OH, pH,  H ) RESULT( GAM )
! !INPUT PARAMETERS:
!
      REAL(fp),       INTENT(IN) :: Radius   ! Radius (cm)
      REAL(fp),       INTENT(IN) :: xvol     ! aerosol volume in air
      REAL(fp),       INTENT(IN) :: n_air    ! Number density of air (#/cm3)
      REAL(fp),       INTENT(IN) :: T        ! Temparature (K)
      REAL(fp),       INTENT(IN) :: cavg     ! Mean thermal velocity
      REAL(fp),       INTENT(IN) :: C_MSA_g  ! Gas-phase MSA concentration
      REAL(fp),       INTENT(IN) :: C_OH     ! Aqueous OH (mol/L)
...
!
      ...
      REAL(fp) :: mf_MSA ! mass fraction of MSA in aerosol
      REAL(fp) :: M_MSA
      REAL(fp) :: k_Mungall ! Mungall et al. 2018 2nd order rate constant for loss of MSA
      REAL(fp) :: D_cm ! diameter in cm
      REAL(fp) :: Na ! avogadro's number
      REAL(fp) :: rho_aq ! density of deliquesced particles

      mf_MSA = C_MSA_g / (xvol * n_air) ! mass fraction of MSA in aerosol
      M_MSA = H%MSA%MW_g

      k_Mungall = 6.2E-13_fp
      D_cm = Radius * 2
      Na= 6.022e23
      rho_aq = xvol * n_air / Na * 28.97 ! density of particles is fraction aerosol volume /
      ! number density of air (molec / cm3) * 
      ! avogadro (molec/mol) * molar mass of air (g / mol) 
      ! I checked these units by making sure gamma should be unitless by the end

      ! From Mungall et al. (2018) equation 5
      GAM = 2 * k_Mungall * D_cm * rho_aq * mf_MSA * Na / (3 * cavg * M_MSA)


    END FUNCTION GAMMA_MSA_AER
