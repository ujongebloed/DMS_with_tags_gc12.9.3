...
!
  ...
  REAL(fp), ALLOCATABLE, PUBLIC :: SO4b(:,:,:)
       ...
       IF ( LSULF ) THEN
          ...
          IF ( LUCX ) THEN
             ...
             IF ( State_Met%InTroposphere(I,J,L) ) THEN
                ...
                SO4b(I,J,L) = Spc(I,J,L,id_SO4b) / AIRVOL(I,J,L)
          ...
          ELSE
             ...
             SO4_NH4_NIT(I,J,L) = ( Spc(I,J,L,id_SO4)    + &
                                    ...
                                    AIRVOL(I,J,L)
             SO4(I,J,L) = Spc(I,J,L,id_SO4) / AIRVOL(I,J,L)
             SO4b(I,J,L)= Spc(I,J,L,id_SO4b) / AIRVOL(I,J,L) ! UAJ
             NH4(I,J,L) = Spc(I,J,L,id_NH4) / AIRVOL(I,J,L)
             NIT(I,J,L) = Spc(I,J,L,id_NIT) / AIRVOL(I,J,L)
          ...
          IF ( SO4_NH4_NIT(I,J,L) > 0e+0_fp ) THEN
             ...
             FRAC_SNA(I,J,L,1) = ( ( Spc(I,J,L,id_SO4) + Spc(I,J,L,id_SO4b))& ! UAJ
!     &                           +     Spc(I,J,L,id_SO4s) )
                                 /   AIRVOL(I,J,L)         ) &
                                 / SO4_NH4_NIT(I,J,L)
...
!     &                           +     Spc(I,J,L,id_NITs) )
       ...
       IF ( State_Diag%Archive_PM25su ) THEN
          State_Diag%PM25su(I,J,L) = ( (SO4(I,J,L)+SO4b(I,J,L)) * SIA_GROWTH)&
...
120       FORMAT( '     - RDAER: Using online SALA SALC' )
    ...
    MSDENS(6) = State_Chm%SpcData(id_SO4b)%Info%Density
    IF ( LUCX ) THEN
...
!
    ...
    ALLOCATE( SO4b( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC ) ! UAJ
    CALL GC_CheckVar( 'aerosol_mod.F90:SO4b', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    SO4b = 0.0_fp
...
!
    ...
    IF ( ALLOCATED( SO4b        ) ) DEALLOCATE( SO4b        )
...
!
    ...
    DO I = 1, State_Grid%NX
       ...
       !--------------------------------------
       ! AerMassSO4b [ug/m3]                    ! UAJ
       !--------------------------------------
       IF ( State_Diag%Archive_AerMassSO4b ) THEN
          State_Diag%AerMassSO4b(I,J,L) = SO4b(I,J,L) * kgm3_to_ugm3
       ENDIF
