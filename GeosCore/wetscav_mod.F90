...
!
    ...
    IF ( SpcInfo%WD_Is_SO2 ) THEN
       ...
       DO I = 1, State_Grid%NX
          ...
          IF ( (SO2s(I,J,L)+SO2sb(I,J,L)) > TINY_FP ) THEN

             ! Limit F
             SO2LOSS      = MIN( H2O2s(I,J,L), (SO2s(I,J,L)+SO2sb(I,J,L) ))
             F(I,J,L)     = F(I,J,L) * SO2LOSS / (SO2s(I,J,L)+SO2sb(I,J,L))
             F(I,J,L)     = MAX(F(I,J,L), 0e+0_fp)

             ! Update saved H2O2 concentration
             H2O2s(I,J,L) = H2O2s(I,J,L) - &
                     ( SO2s(I,J,L) * F(I,J,L) + SO2sb(I,J,L) * F(I,J,L) )
          ...
          SO2sb(I,J,L)    = SO2sb(I,J,L) * ( 1e+0_fp - F(I,J,L) )
          SO2s(I,J,L)     = MAX( SO2s(I,J,L), TINY_FP )
          SO2sb(I,J,L)    = MAX( SO2sb(I,J,L), TINY_FP )
...
!
    ...
    IF ( SpcInfo%WD_Is_SO2 ) THEN
       ...
       IF ( SO2s(I,J,L)+SO2sb(I,J,L) > TINY_FP ) THEN
          ...
          ! Limit RAINFRAC
          SO2LOSS      = MIN( (SO2s(I,J,L)+SO2sb(I,J,L)), H2O2s(I,J,L) )
          RAINFRAC     = SO2LOSS * RAINFRAC / (SO2s(I,J,L)+SO2sb(I,J,L))
          RAINFRAC     = MAX( RAINFRAC, 0e+0_fp )

          ! Update saved H2O2 concentration
          H2O2s(I,J,L) = H2O2s(I,J,L) - &
                  ( SO2s(I,J,L) * RAINFRAC + SO2sb(I,J,L) * RAINFRAC )
          H2O2s(I,J,L) = MAX( H2O2s(I,J,L), TINY_FP )
       ...
       SO2sb(I,J,L)    = SO2sb(I,J,L) * ( 1.0_fp - RAINFRAC )
       SO2s(I,J,L)     = MAX( SO2s(I,J,L), TINY_FP )
       SO2sb(I,J,L)    = MAX( SO2sb(I,J,L), TINY_FP )
...
!
    ...
    REAL(fp),       INTENT(INOUT) :: SO2sb      ! conc's after aqueous rxns
...
!
    ...
    ELSE IF ( SpcInfo%WD_Is_SO2 ) THEN
       ...
       IF ( TK >= 268e+0_fp .AND. SO2s+SO2sb > TINY_FP ) THEN

          ! Adjust WASHFRAC
          SO2LOSS  = MIN( (SO2s+SO2sb), H2O2s )
          WASHFRAC = SO2LOSS * WASHFRAC / (SO2s+SO2sb)
          WASHFRAC = MAX( WASHFRAC, 0e+0_fp )

          ! Deplete H2O2s the same as SO2s (dkh, rjp, bmy, 11/17/05)
          H2O2s = H2O2s - ( SO2s * WASHFRAC + SO2sb * WASHFRAC )
       ...
       SO2sb = SO2sb * ( 1e+0_fp - WASHFRAC )
       SO2s  = MAX( SO2s, TINY_FP )
       SO2sb = MAX( SO2sb, TINY_FP )
...
!
    ...
    DO NW = 1, State_Chm%nWetDep
       ...
       IF ( KIN ) THEN
          ...
          ELSE IF ( N == id_SO2b ) THEN
             Spc(I,J,L,id_SO4b) = Spc(I,J,L,id_SO4b) &
                                 + GAINED * 96e+0_fp / 64e+0_fp 
...
!
    ...
    IF ( FIRST ) THEN
       ...
       IF ( ( id_SO2b > 0 ) .AND. &
            ( SUM(State_Chm%SO2bAfterChem(:,:,:)) < 1e-31 ) ) THEN
          State_Chm%SO2bAfterChem = State_Chm%Species(:,:,:,id_SO2b) &
               * ( AIRMW / State_Chm%SpcData(id_SO2b)%Info%emMW_g )
       ENDIF
