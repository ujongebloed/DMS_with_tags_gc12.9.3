...
!
    ...
    INTEGER, SAVE            :: id_SO4b, id_SO4sb ! UAJ
    ...
    IF ( FIRST ) THEN
       ...
       ENDIF
       IF ( id_SO4b <= 0 ) THEN
          ErrMsg = 'SO4b is an undefined species!'
          CALL GC_Error( ErrMsg, RC, ThisLoc )
          RETURN
       ENDIF
       IF ( id_NH3 <= 0 ) THEN
       ...
       ENDIF
       IF ( id_SO4sb <= 0 ) THEN
          CALL ERROR_STOP( 'SO4sb is an undefined species!', X )
       ENDIF
       IF ( id_SALAAL <= 0 ) THEN
...
110    FORMAT( 'Successfully initialized ISORROPIA code II' )
    ...
    DO I = 1, State_Grid%NX
       ...
       VOL      = State_Met%AIRVOL(I,J,L)
         ...
         !The coarse SSA thermodynamic does not work for offline simulation
            ...
            IF ( N == 1) THEN
               ...
               TSO4 = ((Spc(I,J,L,id_SO4)+Spc(I,J,L,id_SO4b))+&
                       Spc(I,J,L,id_SALA)*0.08*AlkR) &
            ...
            ELSE
               ...
               TSO4 = (Spc(I,J,L,id_SO4sb) + Spc(I,J,L,id_SO4s)) &
       ...
       IF (N==1) THEN
          ...
          DEN_SAV    = ( (Spc(I,J,L,id_SO4)+Spc(I,J,L,id_SO4b)) /&
                         96e+0_fp*2e+0_fp + &    
