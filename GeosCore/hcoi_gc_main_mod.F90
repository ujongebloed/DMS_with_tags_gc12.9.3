...
200         FORMAT( 12x, ' SO2b_AChem: Min = ', es15.9, ', Max = ', es15.9 )
         ENDIF
      ELSE
      ...
      ! Check if variable is in file
      IF ( FOUND ) THEN
         State_Chm%SO2bAfterChem = Ptr3D
         IF ( Input_Opt%amIRoot ) THEN
            WRITE(6,*) 'Initialize dry deposited nitrogen from restart file'
            WRITE(6,200) MINVAL( State_Chm%SO2bAfterChem(:,:,:) ), &
                         MAXVAL( State_Chm%SO2bAfterChem(:,:,:) )
201         FORMAT( 12x, ' SO2b_AChem: Min = ', es15.9, ', Max = ', es15.9 )
         ENDIF
      ELSE
         State_Chm%SO2bAfterChem = 0e+0_fp
         IF ( Input_Opt%amIRoot ) THEN
            WRITE(6,*) 'SO2B_AFTERCHEM  not found in restart, set to zero'
         ENDIF
      ENDIF
