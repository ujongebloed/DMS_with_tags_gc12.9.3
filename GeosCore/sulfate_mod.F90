...
!
  ...
  ! UAJ ADDED VARS
  INTEGER                :: id_SO2b,   id_SO4b,   id_SO4d1b
  INTEGER                :: id_SO4d2b, id_SO4d3b, id_SO4d4b
  INTEGER                :: id_SO4sb,  id_SO4aqb
  ...
  INTEGER                :: DRYNITd4
  ! UAJ ADDED VARS
  INTEGER                :: DRYSO4sb,  DRYSO4d1b, DRYSO4d2b
  INTEGER                :: DRYSO4d3b, DRYSO4d4b
...
!
    ...
    PSO2b_DMS   = 0e+0_fp ! UAJ
    PMSA_DMS   = 0e+0_fp
    PSO4_SO2   = 0e+0_fp
    PSO4b_SO2b   = 0e+0_fp ! UAJ
    PSO4_ss    = 0e+0_fp
    PSO4_ssb   = 0e+0_fp ! UAJ
    PNITs      = 0e+0_fp
    PSO4_dust  = 0e+0_fp     ! tdf 04/17/08
    PSO4_dustb  = 0e+0_fp     ! UAJ
    ...
    IF ( FullRun ) THEN
       ...
       ! SO4sb [kg] gravitational settling
       IF ( id_SO4sb > 0 ) THEN
          CALL GRAV_SETTLING( Input_Opt, State_Chm, State_Diag, State_Grid, &
                              State_Met, id_SO4sb,   RC )
          IF ( prtDebug ) THEN
             CALL DEBUG_MSG( '### CHEMSULFATE: GRAV_SET, SO4Sb' )
          ENDIF
       ENDIF
       ...
       IF ( LDSTUP ) THEN
          ...
          ! SO4d1b [kg] gravitational settling            ! UAJ
          IF ( id_SO4d1b > 0 ) THEN
             CALL GRAV_SETTLING( Input_Opt, State_Chm, State_Diag, State_Grid, &
                                 State_Met, id_SO4d1b,  RC )
             IF ( prtDebug ) THEN
                CALL DEBUG_MSG( '### CHEMSULFATE: GRAV_SET, SO4d1b')
             ENDIF
          ENDIF
          ...
          ! SO4d2b [kg] gravitational settling                      ! UAJ
          IF ( id_SO4d2b > 0 ) THEN 
             CALL GRAV_SETTLING( Input_Opt, State_Chm, State_Diag, State_Grid, &
                                 State_Met, id_SO4d2b,  RC )
             IF ( prtDebug ) THEN
                CALL DEBUG_MSG( '### CHEMSULFATE: GRAV_SET, SO4d2b')
             ENDIF
          ENDIF
          ...
          ! SO4d3b [kg] gravitational settling     ! UAJ
          IF ( id_SO4d3b > 0 ) THEN
             CALL GRAV_SETTLING( Input_Opt, State_Chm, State_Diag, State_Grid, &
                                 State_Met, id_SO4d3b,  RC )
             IF ( prtDebug ) THEN
                CALL DEBUG_MSG( '### CHEMSULFATE: GRAV_SET, SO4d3b')
             ENDIF
          ENDIF
          ...
          ! SO4d4b [kg] gravitational settling   ! UAJ
          IF ( id_SO4d4b > 0 ) THEN
             CALL GRAV_SETTLING( Input_Opt, State_Chm, State_Diag, State_Grid, &
                                 State_Met, id_SO4d4b,  RC )
             IF ( prtDebug ) THEN
                CALL DEBUG_MSG( '### CHEMSULFATE: GRAV_SET, SO4d4b')
             ENDIF
          ENDIF
...
!
    ...
    IS_UPTAKE_SPC =  ( ( N /= id_SO4s ) .and. ( N /= id_NITs ) .and. &
            ( N /= id_SO4sb) )
...
!
    ...
    DO I = 1, State_Grid%NX
       ...
       PSO2b_DMS(I,J,L) =  DMS0 - DMS - PMSA_DMS(I,J,L)  ! UAJ
       ...
       ! Store P(SO2) from DMS + OH [kg S/s]  ! UAJ
       IF ( State_Diag%Archive_ProdSO2bfromDMSandOH ) THEN
          State_Diag%ProdSO2bfromDMSandOH(I,J,L) = XOH / DTCHEM
       ENDIF
  
       ! Store P(SO2) from DMS + NO3 [kg S/s]   ! UAJ
       IF ( State_Diag%Archive_ProdSO2bfromDMSandNO3 ) THEN
          State_Diag%ProdSO2bfromDMSandNO3(I,J,L) = XN3 / DTCHEM
       ENDIF

       ! Store P(SO2) from DMS + NO3 [kg S/s]   ! UAJ
       IF ( State_Diag%Archive_ProdSO2bfromDMS ) THEN
          State_Diag%ProdSO2bfromDMS(I,J,L) = &
               ( PSO2b_DMS(I,J,L) * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
       ENDIF

       ! Store P(MSA) from DMS [kg S/s]   ! UAJ
...
!
    ...
    ! UAJ ADDED VARS
    REAL(fp)              :: L1b,    Ldb,     SO20b,  SO2_cdb
    REAL(fp)              :: L2Sb,   SO2_ssb, PSO4Eb, PSO4Fb
    REAL(fp)              :: L3Sb,   SO4nssb, LSTOTb, aSO4b
    REAL(fp)              :: SRb,    SO2_srb
    REAL(fp)              :: PSO4d_totb, PH2SO4d_totb
    REAL(fp)              :: SO2_gasb,   PH2SO4_gasb, H2SO4_cdb
    REAL(fp)              :: H2SO4_gasb
    ...
    REAL(fp)              :: TMP
    ! UAJ ADDED VARS
    REAL(fp)              :: L5Sb,   SRo3b,   SRhobrb, L3_1b, L4Sb
    REAL(fp)              :: L5S_1b, L3S_1b,  HSO3aqb, SO3aqb
    REAL(fp)              :: LSTOT0b
    REAL(fp)              :: SO2_ss0b
    ...
    ! UAJ ADDED VARS
    REAL(fp)              :: L6Sb,   SRhoclb, L6S_1b
   ! might delete L5Sb, L6Sb, L5S_1b, and L6S_1b 
    ...
    REAL(fp)              :: PSO4_db (NDSTBIN) ! UAJ
    REAL(fp)              :: PNIT_d  (NDSTBIN)
    REAL(fp)              :: PH2SO4_d(NDSTBIN)
    REAL(fp)              :: PH2SO4_db(NDSTBIN) ! UAJ
    REAL(fp)              :: KTN(NDSTBIN)
    REAL(fp)              :: KTS(NDSTBIN)
    REAL(fp)              :: KTH(NDSTBIN)
    ...
    !=================================================================
    IF ( id_H2O2 < 0 .or. id_SO2 < 0 .or. id_SO2b < 0 ) RETURN ! UAJ
    ...
    SO2sb       => State_Chm%SO2bAfterChem ! UAJ
    ...
    !$OMP PRIVATE( SO20b, SO2_cdb, L1b, Ldb, L2Sb, L3Sb, SO2_ssb, PSO4Eb   ) &
    ...
    !$OMP PRIVATE( PSO4Fb, SO4nssb, LSTOTb, aSO4b, SO2_srb, SRb            ) &
    ...
    !$OMP PRIVATE( SO2_gasb, H2SO4_cdb, H2SO4_gasb                         ) &
    !$OMP PRIVATE( KTH, H2SO4_cd, H2SO4_gas, Ki                            ) &
    !$OMP PRIVATE( PSO4d_totb, PH2SO4d_totb, L5Sb, SRo3b, SRhobrb          ) &
    ...
    !$OMP PRIVATE( L3_1b, L3S_1b, L5S_1b                                   ) &
    !$OMP PRIVATE( HSO3aqb, SO3aqb, LSTOT0b                                ) &
    !$OMP PRIVATE( SO4H1_vv, SO4H2_vv, LSTOT0, SO2_ss0, rSIV,fupdateHOBr_0 ) &
    !$OMP PRIVATE( SO2_ss0b, L4Sb, L6Sb, L6S_1b, SRhoclb                   ) &
    ...
    DO I = 1, State_Grid%NX
       ...
       IF ( RK > 0.e+0_fp ) THEN
          ...
          SO2_cdb = ( SO20b  * EXP( -RKT ) ) + &
                   ( PSO2b_DMS(I,J,L) * ( 1.e+0_fp - EXP( -RKT ) ) / RKT ) ! UAJ

          L1     = ( SO20 - SO2_cd ) * RK1/RK
          L1b    = ( SO20b - SO2_cdb + PSO2b_DMS(I,J,L) ) * RK1/RK ! UAJ

          Ld     = ( SO20 - SO2_cd ) * RK2/RK
          Ldb    = ( SO20b - SO2_cdb + PSO2b_DMS(I,J,L) ) * RK2/RK ! UAJ
       ...
       IF ( LDSTUP ) THEN
          ...
          H2SO4_cdb = SO20b * ( 1.e+0_fp - EXP( -RKT ) ) ! UAJ
       ...
       IF  ( ( ALK    > MINDAT )  .AND. &
          ...
          SO2_ssb = SO2_ss * (SO2_cdb/(SO2_cd + SO2_cdb))
          SO2_ss  = SO2_ss * (SO2_cd /(SO2_cd + SO2_cdb))

          PSO4Eb  = PSO4E * (SO2_cdb/(SO2_cd + SO2_cdb))
          PSO4E   = PSO4E * (SO2_cd /(SO2_cd + SO2_cdb))

          PSO4Fb  = PSO4F * (SO2_cdb/(SO2_cd + SO2_cdb))
          PSO4F   = PSO4F * (SO2_cd /(SO2_cd + SO2_cdb))
       ...
       IF ( LDSTUP .and. FullRun ) THEN
          ...
          IF  ( ( ALK    > MINDAT )  .AND. &
             ...
             CALL DUST_CHEM( I,         J,         L,            &
                             ALK_d,SO2_ss+SO2_ssb, H2SO4_cd+H2SO4_cdb, &
                             KTS,       KTN,       KTH,          &
                             SO2_gas, H2SO4_gas, &
                             PSO4_d,     &
             ...
             SO2_gasb = SO2_gas * (SO2_ssb/(SO2_ss + SO2_ssb))
             SO2_gas  = SO2_gas * (SO2_ss /(SO2_ss + SO2_ssb))

             PSO4_db  = PSO4_d * (SO2_ssb/(SO2_ss + SO2_ssb))
             PSO4_d   = PSO4_d * (SO2_ss /(SO2_ss + SO2_ssb))

             H2SO4_gasb = H2SO4_gas * (SO2_ssb/(SO2_ss + SO2_ssb))
             H2SO4_gas  = H2SO4_gas * (SO2_ss /(SO2_ss + SO2_ssb))

             PH2SO4_db  = PH2SO4_d * (SO2_ssb/(SO2_ss + SO2_ssb))
             PH2SO4_d   = PH2SO4_d * (SO2_ss /(SO2_ss + SO2_ssb))

             ! tdf "SO2_ss" is SO2 mixing ratio remaining after interaction
             ! with dust
             SO2_ss = SO2_gas
             SO2_ssb = SO2_gasb ! UAJ

             ! tdf "H2SO4_cd" is H2SO4 remaining after interaction with dust
             H2SO4_cd = H2SO4_gas
             H2SO4_cdb = H2SO4_gasb ! UAJ
          ...
          ELSE
             ...
             DO IBIN = 1, NDSTBIN
                ...
                PSO4_db    (IBIN)   = 0.e+0_fp ! UAJ
                PH2SO4_d  (IBIN)   = 0.e+0_fp
                PH2SO4_db  (IBIN)   = 0.e+0_fp ! UAJ
       ...
       L3S_1b  = 0.e+0_fp !(qjc, 04/10/16)
       L5S     = 0.e+0_fp !(qjc, 04/10/16)
       L5S_1   = 0.e+0_fp !(qjc, 04/10/16)
       L5S_1b  = 0.e+0_fp !(qjc, 04/10/16)
       KaqO2   = 0.e+0_fp
       L4      = 0.e+0_fp
       L4S     = 0.e+0_fp
       L4Sb    = 0.e+0_fp
	  ...
	  ! Use a cloud scavenging ratio of 0.7
          ...
          SO4nssb =  Spc(I,J,L,id_SO4b) * State_Met%AIRDEN(I,J,L) * & ! UAJ
                     0.7e+0_fp / ( AIRMW * LWC ) +                 &  
                     Spc(I,J,L,id_SO4sb) * State_Met%AIRDEN(I,J,L)  &
                     / ( AIRMW * LWC)
          ...
          ! Calculate cloud pH
            CALL GET_HPLUS( SO4nss, SO4nssb, TNH3, TNO3, SO2_ss, SO2_ssb, & ! UAJ
                             CL, TNA, TDCA, TFA, TAA, TK,              &
          ...
          CALL AQCHEM_SO2( LWC,     TK,    PATM,    SO2_ss+SO2_ssb, H2O20, &
                           O3,      HPLUS, MnII,    FeIII, &
                           KaqH2O2, KaqO3, KaqO3_1, KaqO2, &
                           HSO3aq,  SO3aq )
          HSO3aqb = HSO3aq * (SO2_ssb/(SO2_ssb + SO2_ss))
          HSO3aq  = HSO3aq * (SO2_ss /(SO2_ssb + SO2_ss))

          SO3aqb = SO3aq * (SO2_ssb/(SO2_ssb + SO2_ss))
          SO3aq  = SO3aq * (SO2_ss /(SO2_ssb + SO2_ss))
          ...
          XX  = ( SO2_ss+SO2_ssb - H2O20 ) * KaqH2O2 * DTCHEM 
          ...
          IF ( IS_SAFE_EXP( XX ) .and. ABS( XX ) > 0e+0_fp ) THEN
             ...
             ! Loss by H2O2
             L2S = (SO2_ss+SO2_ssb) * H2O20 * ( L2 - 1.e+0_fp ) / &
                   ( ((SO2_ss+SO2_ssb) * L2) - H2O20 )

           ! UAJ multiply L2S (SO2 lost to H2O2) by fraction SO2 vs SO2b   
             L2Sb = L2S * SO2_ssb / (SO2_ss + SO2_ssb)
             L2S  = L2S * SO2_ss  / (SO2_ss + SO2_ssb)
          ELSE
             ...
             IF ( XX > 0.e+0_fp ) THEN
                L2S  = H2O20 * SO2_ss / (SO2_ss+SO2_ssb) ! UAJ added this mult
                L2Sb = H2O20 * SO2_ssb / (SO2_ss+SO2_ssb) ! term to partition
             ELSE IF ( XX < 0.e+0_fp) THEN
                L2S = SO2_ss
                L2Sb = SO2_ssb
             ELSE
                !(qjc, 04/10/16) different solution when SO2_ss = H2O20
                L2S = (SO2_ss+SO2_ssb) - 1/(KaqH2O2*DTCHEM+1/(SO2_ss+SO2_ssb))
          ...
          ! Argument of the exponential
          XX = ( SO2_ss+SO2_ssb - O3 ) * KaqO3 * DTCHEM
          ...
          IF ( IS_SAFE_EXP( XX ) .and. ABS( XX ) > 0e+0_fp ) THEN
             ...
             ! Loss by O3
             L3S = (SO2_ss+SO2_ssb) * O3 * (L3 - 1.e+0_fp)/ & 
                     (((SO2_ss+SO2_ssb) * L3) - O3)
            
             L3Sb = L3S * SO2_ssb / (SO2_ss + SO2_ssb)
             L3S  = L3S * SO2_ss  / (SO2_ss + SO2_ssb)
          ELSE

             ! Follow the same logic for L3S as described in
             ! Jintai Lin's note above (bmy, 4/28/10)
             IF ( XX > 0.e+0_fp ) THEN
                L3S  = O3 * SO2_ss  / (SO2_ss+SO2_ssb) ! UAJ added SO2 term
                L3Sb = O3 * SO2_ssb / (SO2_ss+SO2_ssb)
             ELSE IF ( XX < 0.e+0_fp) THEN
                L3S = SO2_ss
                L3Sb = SO2_ssb
             ELSE
                !(qjc, 04/10/16) different solution when SO2_ss = O3
                L3S = (SO2_ss+SO2_ssb) - 1/(KaqO3*DTCHEM+1/(SO2_ss+SO2_ssb))
          ...
          ! Argument of the exponential
          XX = ( SO2_ss+SO2_ssb - O3 ) * KaqO3_1 * DTCHEM
          ...
          IF ( IS_SAFE_EXP( XX ) .and. ABS( XX ) > 0e+0_fp ) THEN
             ...
             ! Loss by O3
             L3S_1 = (SO2_ss+SO2_ssb) * O3*(L3_1 - 1.e+0_fp)/ & 
                     (((SO2_ss+SO2_ssb)*L3_1)-O3)
            
             L3S_1b = L3S_1 * SO2_ssb / (SO2_ss + SO2_ssb)
             L3S_1  = L3S_1 * SO2_ss  / (SO2_ss + SO2_ssb)
          ELSE

             ! Follow the same logic for L3S_1 as described in
             ! Jintai Lin's note above
             IF ( XX > 0.e+0_fp ) THEN
                L3S_1 = O3 * SO2_ss / (SO2_ss + SO2_ssb)
                L3S_1b = O3 * SO2_ssb / (SO2_ss + SO2_ssb)
             ELSE IF ( XX < 0.e+0_fp) THEN
                L3S_1 = SO2_ss
                L3S_1b = SO2_ssb
             ELSE
                !(qjc, 04/10/16) different solution when SO2_ss = O3
                L3S_1 = SO2_ss+SO2_ssb - 1/(KaqO3_1*DTCHEM+1/(SO2_ss+SO2_ssb)) 
          ...
          IF ( Input_Opt%LMETALCATSO2 ) THEN
             ...
             IF ( IS_SAFE_EXP( XX ) ) THEN
                ...
                ! Loss by O2
                L4S = (SO2_ss+SO2_ssb) * (1.e+0_fp - L4)
               
                L4Sb = L4S * SO2_ssb / (SO2_ss + SO2_ssb)
                L4S  = L4S * SO2_ss  / (SO2_ss + SO2_ssb) 
             ELSE
                L4S = SO2_ss
                L4Sb = SO2_ssb
             ENDIF
          ...
          L5S   = (SO4H1_vv + SO4H2_vv) * SO2_ss / &
                  (SO2_ss + SO2_ssb)
          L5S_1 = SO4H2_vv * SO2_ss / &
                  (SO2_ss + SO2_ssb)
          L6S   = SO4H3_vv + SO4H4_vv * SO2_ss / &
                  (SO2_ss + SO2_ssb)
          L6S_1 = SO4H3_vv * SO2_ss / &
                  (SO2_ss + SO2_ssb)

          L5Sb   = (SO4H1_vv + SO4H2_vv) * SO2_ssb / &
                  (SO2_ss + SO2_ssb)
          L5S_1b = SO4H2_vv * SO2_ssb / &
                  (SO2_ss + SO2_ssb)
          L6Sb   = SO4H3_vv + SO4H4_vv * SO2_ssb / &
                  (SO2_ss + SO2_ssb)
          L6S_1b = SO4H3_vv * SO2_ssb / &
                  (SO2_ss + SO2_ssb)

        
          ! make sure sulfate produced is less than SO2 available
          ! (qjc, 06/20/16)
          IF ((L5S + L5Sb) > (SO2_ss + SO2_ssb) ) THEN
             fupdateHOBr_0 = (SO2_ss+SO2_ssb)/(L5S+L5Sb)
             L5S   = SO2_ss
             L5Sb  = SO2_ssb
             L5S_1 = SO2_ss * L5S_1/L5S
             L5S_1b = SO2_ssb* L5S_1b/L5Sb
          ELSE
             L5S = L5S
             L5Sb = L5Sb
             L5S_1 = L5S_1
          ...
          IF ((L6S + L6Sb) > (SO2_ss+SO2_ssb)) THEN
             fupdateHOCl_0 = (SO2_ss+SO2_ssb)/(L6S+L6Sb)
             L6S   = SO2_ss
             L6Sb  = SO2_ssb
             L6S_1 = SO2_ss * L6S_1/L6S
             L6S_1b = SO2_ssb * L6S_1b/L6Sb
          ELSE
             L6S = L6S
             L6Sb = L6Sb
             L6S_1 = L6S_1
          ...
          LSTOT0b = L2Sb + L3Sb + L4Sb + L5Sb + L6Sb ! (qjc, 11/04/16)

          ! make sure sulfate produced is less than SO2 available
          ! (qjc, 06/20/16)
          IF ((LSTOT0+LSTOT0b) > (SO2_ss+SO2_ssb)) THEN
             ...
             L2Sb   = SO2_ssb * L2Sb   / LSTOT0b
             L3Sb   = SO2_ssb * L3Sb   / LSTOT0b
             L4Sb   = SO2_ssb * L4Sb   / LSTOT0b
             L5Sb   = SO2_ssb * L5Sb   / LSTOT0b
             L3S_1b = SO2_ssb * L3S_1b / LSTOT0b
             L5S_1b = SO2_ssb * L5S_1b / LSTOT0b
             L6S_1b = SO2_ssb * L6S_1b / LSTOT0b

             ! This is the ratio used to calculate the actual removal of
             ! HOBr by SO2 for use in gckpp_HetRates.F90 (qjc, 06/20/16)
             State_Chm%fupdateHOBr(I,J,L) = fupdateHOBr_0 * (SO2_ss+SO2_ssb)/&
                                            ( LSTOT0 + LSTOT0b)
             State_Chm%fupdateHOCl(I,J,L) = fupdateHOCl_0 * (SO2_ss+SO2_ssb)/&
                                            ( LSTOT0 + LSTOT0b)
          ...
          IF ( H2O20 > (SO2_ss+SO2_ssb) + 1e-9_fp ) THEN
             BULK = 1
          ELSEIF( LWC < 0.1e-6_fp ) THEN !10^-6 coversion from g/m3 --> m3/m3
             SIZE_RES = 1
          ELSEIF( gno3 > NH3 ) THEN
             IF ( (SO2_ss+SO2_ssb) >= 5.e-9_fp  .and. &
                  H2O20  >= (SO20+SO20b)   )          &
                  BULK    = 1
             IF ( LWC    >= 0.3e-6_fp         .and. &
                  (SO2_ss+SO2_ssb) >= 3.e-9_fp  .and. &
                  H2O20  >= (SO2_ss+SO2_ssb) )        &
                  BULK    = 1
          ...
          ELSEIF( LWC    >= 0.5e-6_fp ) THEN
             IF ( H2O20  >= (0.9e+0_fp * (SO2_ss+SO2_ssb)) ) &
                  BULK    = 1
             IF ( NH3    <= 1.e-9_fp          .and. &
                  ALKds  >= 5.e+0_fp          .and. &
                  (SO2_ss+SO2_ssb) <= 10.e-9_fp )   &
                  BULK    = 1
          ELSEIF( LWC    >= 0.3e-6_fp ) THEN
             ...
             IF ( ALKds  >= 3.e+0_fp          .and. &
                  NH3 <= 10e-9_fp             .and. &
                  (SO2_ss+SO2_ssb) <= 5e-9_fp )     &
                  BULK    = 1
             IF ( ALKds  >= 5.e+0_fp          .and. &
                  NH3    <= 10.e-9_fp         .and. &
                  (SO2_ss+SO2_ssb) <= 5.e-9_fp )    &
                  BULK    = 1
             IF ( (SO2_ss+SO2_ssb) >= 1.5e-9_fp .and. &
                  H2O20  >= (SO2_ss+SO2_ssb) )      &
                  BULK    = 1
          ...
          ELSEIF( LWC    >= 0.1e-6_fp ) THEN
             ...
             IF ( gno3   <= 7.e-9_fp          .and. &
                  NH3    >= (gno3 + 3.e-9_fp) .and. &
                  ALKds  >= 4.e+0_fp          .and. &
                  (SO2_ss+SO2_ssb) <= 9.e-9_fp  )   &
                  BULK    = 1
             IF ( ALKds  >= 3.e+0_fp          .and. &
                  NH3    <= 3.e-9_fp          .and. &
                  (SO2_ss+SO2_ssb) <= 4.e-9_fp )    &
                  BULK    = 1
             IF ( ALKds  >= 5.e+0_fp          .and. &
                  (SO2_ss+SO2_ssb) <= 5.e-9_fp  .and. &
                  NH3    <= 7.e-9_fp )              &
                  BULK    = 1
          ...
          IF ( SIZE_RES == 1 .AND. State_Met%IsWater(I,J) .AND. &
             ...
             LSTOT = (L2S + L3S + L4S + L5S + L6S ) / FC !(qjc, 06/20/16) ! UAJ
             LSTOTb = (L2Sb + L3Sb + L4Sb + L5Sb + L6Sb + L7S + L8S) / FC !(qjc, 06/20/16) ! UAJ
             ...
             SO2_srb = MAX( SO2_ssb - ( L2Sb + L3Sb + L4Sb + L5Sb + L6Sb ),&
                     MINDAT)

             CALL HET_DROP_CHEM( I,    J,   L,      LSTOT+LSTOTb, SSCvv, &
                                 aSO4+ aSO4b,NH3, SO2_sr+SO2_srb, H2O20,&
                                 GNO3,  SR, &
                                 Input_Opt, State_Met, State_Chm )
             
             SRb = SR * (SO2_srb/(SO2_sr + SO2_srb))
             SR  = SR * (SO2_sr /(SO2_sr + SO2_srb))
             ...
             IF ( TMP > 0.0_fp ) THEN
                  ...
                  SRhobrb   = SRb * f_srhobr
                  SRhoclb = SRb * f_srhocl
                  SRo3b   = SRb * (1-f_srhobr-f_srhocl)
          ...
          SRb     =  SRb     * FC
          SRo3b   =  SRo3b   * FC !(qjc, 04/10/16)
          SRhobrb =  SRhobrb * FC !(qjc, 04/10/16)
          SRhoclb =  SRhoclb * FC
          ...
          SO2_ssb = MAX( SO2_ssb - ( L2Sb+L3Sb+L4Sb+L5Sb+L6Sb+SRb ), MINDAT )
          H2O20  = MAX( H2O20  - (L2S+L2Sb),                 MINDAT )

          ! Factor to calculate effective SO3 and HSO3 in aqchem_so2 to
          ! be used in HOBr+HSO3/SO3 (qjc, 06/20/16)
          rSIV = (SO2_sr+SO2_srb)/(SO2_ss0+SO2_ss0b)
          ...
          State_Chm%HSO3_AQb(I,J,L) = HSO3aqb*(1+rSIV)/2  
          State_Chm%SO3_AQb(I,J,L)  = SO3aqb *(1+rSIV)/2
          ...
          PSO4_ss (I,J,L) = PSO4F
          PSO4b_SO2b(I,J,L) = LSTOTb + PSO4Eb
          PSO4_ssb (I,J,L) = PSO4Fb
       ...
       ELSE
          ...
          SO2sb(I,J,L ) = MAX( SO2_ssb, 1.0e-32_fp )
          L2S          = 0.e+0_fp
          L2Sb         = 0.e+0_fp
          L3S          = 0.e+0_fp
          L3Sb         = 0.e+0_fp
          L3S_1        = 0.e+0_fp !(qjc, 11/04/16)
          L3S_1b       = 0.e+0_fp !(qjc, 11/04/16)
          L4S          = 0.e+0_fp
          L4Sb         = 0.e+0_fp
          L5S          = 0.e+0_fp !(qjc, 11/04/16)
          L5Sb         = 0.e+0_fp !(qjc, 11/04/16)
          L5S_1        = 0.e+0_fp !(qjc, 11/04/16)
          L5S_1b       = 0.e+0_fp !(qjc, 11/04/16)
          L6S          = 0.e+0_fp
          L6Sb         = 0.e+0_fp
          L6S_1        = 0.e+0_fp
          L6S_1b       = 0.e+0_fp
          ...
          State_Chm%HSO3_AQb(I,J,L)     = 1.0e-32_fp
          State_Chm%SO3_AQb(I,J,L)      = 1.0e-32_fp
       ...
       PSO4b_SO2b(I,J,L) = L1b + L2Sb + L3Sb + L4Sb + L5Sb + PSO4Eb + SRb &
               + L6Sb + L7S + L8S
       ...
       IF ( LDSTUP ) THEN
          ...
          DO IBIN = 1, NDSTBIN
             ...
             PSO4_dustb(I,J,L,IBIN) = PSO4_db(IBIN) + PH2SO4_db(IBIN)
          ...
          DO IBIN = 1, NDSTBIN
             ...
             PSO4b_SO2b(I,J,L) =  PSO4b_SO2b(I,J,L) - PH2SO4_db(IBIN)
          END DO
       ...
       IF ( FullRun ) THEN
          ...
          ! P(SO4b) from gas-phase oxidation [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromGasPhase ) THEN
             State_Diag%ProdSO4bfromGasPhase(I,J,L) = &
                  ( L1b  * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          ! P(SO4b) from aqueous-phase oxidation with H2O2 [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromH2O2inCloud ) THEN
             State_Diag%ProdSO4bfromH2O2inCloud(I,J,L) = &
                  ( L2Sb * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          ! P(SO4b) from aqueous-phase oxidation with O3 [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromO3InCloud ) THEN
             State_Diag%ProdSO4bfromO3InCloud(I,J,L) = &
                  ( ( L3Sb + SRo3b ) * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          ! P(SO4b) from aqueous-phase oxidation with HOBr [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromHOBrInCloud ) THEN
             State_Diag%ProdSO4bfromHOBrInCloud(I,J,L) = &
                  ((L5Sb + SRhobrb) * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          ! P(SO4b) from aqueous-phase oxidation with O2 metal-catalyzed
          ! [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromO2InCloudMetal ) THEN
             State_Diag%ProdSO4bfromO2InCloudMetal(I,J,L) = &
                  ( L4Sb * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          ! P(SO4b) from O3 in sea salt aerosol [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromO3inSeaSalt ) THEN
             State_Diag%ProdSO4bfromO3inSeaSalt(I,J,L) = &
                  ( ( PSO4Eb + PSO4Fb) * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          ! P(SO4b) by SRo3 [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromSRO3 ) THEN
             State_Diag%ProdSO4bfromSRO3(I,J,L) = &
                  ( SRo3b * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          ! P(SO4b) by SRhobr [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromSRHOBr ) THEN
             State_Diag%ProdSO4bfromSRHOBr(I,J,L) = &
                  ( SRhobrb * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          ! P(SO4) by o3s [kg S/s]
          IF ( State_Diag%Archive_ProdSO4bfromO3s ) THEN
             State_Diag%ProdSO4bfromO3s(I,J,L) = &
                  ( L3S_1b * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
          ENDIF
          ...
          IF ( LDSTUP ) THEN
             ...
             ! P(SO4b) from O3 oxidation on dust aerosols [kg S/s]
             IF ( State_Diag%Archive_ProdSO4bfromOxidationOnDust ) THEN
                State_Diag%ProdSO4bfromOxidationOnDust(I,J,L) = &
                     ( PSO4d_totb * State_Met%AD(I,J,L) / TCVV_S ) / DTCHEM
             ENDIF
             ...
             ! P(SO4) from uptake of H2SO4 on dust aerosols [kg S/s]
             IF ( State_Diag%Archive_ProdSO4bfromUptakeOfH2SO4g ) THEN
                State_Diag%ProdSO4bfromUptakeOfH2SO4g(I,J,L) = &
                     ( PH2SO4d_totb * State_Met%AD(I,J,L) / TCVV_S )/DTCHEM
             ENDIF
...
! !INTERFACE:
!
    SUBROUTINE GET_HPLUS( SO4nss, SO4nssb, TNH3, TNO3, SO2, SO2b, CL,&
                          TNA, TDCA, TFA, &
...
!
    ...
    REAL(fp),  INTENT(IN)    :: SO4nssb ! Total nss sulfate mixing ratio [M]
    ...
    REAL(fp),  INTENT(IN)    :: SO2b   ! SO2 mixing ratio [v/v]
...
!
    ...
    D = (2.e+0_fp*(SO4nss+SO4nssb)) - TNA - (2.e+0_fp*TDCA)
    ...
    DO WHILE ( ABS(ipH-newpH) .gt. MINVAL )
       ...
       fHSO3  = kSO21 ( PRES, T, LWC, nHPLUS, (SO2+SO2b) )
       
       fSO3 = kSO22 ( PRES, T, LWC, nHPLUS, (SO2+SO2b) )
       ...
       dHSO3  = dkSO21 ( PRES, T, LWC, nHPLUS, (SO2+SO2b) )
       
       dSO3 = dkSO22 ( PRES, T, LWC, nHPLUS, (SO2+SO2b) )
...
!
    ...
    REAL(fp)          :: SO4b, SO40b, SO4sb, SO40sb
    REAL(fp)          :: PSO4d, SO40_dust ! tdf 04/07/08
    REAL(fp)          :: PSO4db, SO40_dustb ! tdf 04/07/08

    ! Arrays
    REAL(fp)          :: SO4d (NDSTBIN)  ! tdf 04/07/08
    REAL(fp)          :: SO4db (NDSTBIN)  ! tdf 04/07/08
    REAL(fp)          :: SO40d(NDSTBIN)  ! tdf 04/07/08
    REAL(fp)          :: SO40db(NDSTBIN)  ! tdf 04/07/08
    ...
    IF ( id_SO4 < 0 .or. id_SO4s < 0 .or. id_SO4b < 0 .or. id_SO4sb<0 ) RETURN
    ...
    !$OMP PARALLEL DO       &
    !$OMP DEFAULT( SHARED ) &
    !$OMP PRIVATE( I, J, L, SO4, SO4b, SO4s, SO4sb, SO40, SO40b      ) &
    !$OMP PRIVATE( SO40s, SO40sb                                     ) &
    !$OMP PRIVATE( SO4d, SO4db, SO40d, SO40db, SO40_dust, SO40_dustb ) &
    !$OMP PRIVATE( IBIN, PSO4d, PSO4db, IDTRC                        ) &
#ifdef APM
    ...
    DO I = 1, State_Grid%NX
       ...
       IF ( LDSTUP ) THEN
          ...
          SO40db(1) = Spc(I,J,L,id_SO4d1b)
          SO40db(2) = Spc(I,J,L,id_SO4d2b)
          SO40db(3) = Spc(I,J,L,id_SO4d3b)
          SO40db(4) = Spc(I,J,L,id_SO4d4b)
       ...
       IF ( SO4b  < SMALLNUM ) SO4b  = 0e+0_fp
       IF ( SO4sb < SMALLNUM ) SO4sb = 0e+0_fp 

       ! Final concentrations [v/v]
       Spc(I,J,L,id_SO4)  = SO4
       Spc(I,J,L,id_SO4b)  = SO4b
       Spc(I,J,L,id_SO4s) = SO4s
       Spc(I,J,L,id_SO4sb) = SO4sb
...
!APM_GanLuo+
       ...
       IF ( LDSTUP ) THEN
          DO IBIN = 1, NDSTBIN
             SO40_dust = SO4d(IBIN)
             SO40_dustb = SO4db(IBIN)
             IF ( (SO40_dust+SO40_dustb) < SMALLNUM ) SO40_dust = 0e+0_fp
             IF ( (SO40_dust+SO40_dustb) < SMALLNUM ) SO40_dustb = 0e+0_fp
             Spc(I,J,L,IDTRC(IBIN)) = SO40_dust+SO40_dustb ! dust sulfate
          ENDDO
       ENDIF    !tdf end of if ( LDSTUP) condition 
...
!
    ...
    ALLOCATE( PSO2b_DMS( State_Grid%NX, State_Grid%NY, State_Grid%MaxChemLev ), &
              STAT=RC )
    CALL GC_CheckVar( 'sulfate_mod.F90:PSO2b_DMS', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    PSO2b_DMS = 0e+0_fp
    ...
    ALLOCATE( PSO4b_SO2b( State_Grid%NX, State_Grid%NY, State_Grid%MaxChemLev ), &
              STAT=RC )
    CALL GC_CheckVar( 'sulfate_mod.F90:PSO4b_SO2b', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    PSO4b_SO2b = 0e+0_fp
    ...
    ALLOCATE( PSO4_ssb( State_Grid%NX, State_Grid%NY, State_Grid%MaxChemLev ), &
              STAT=RC )
    CALL GC_CheckVar( 'sulfate_mod.F90:PSO4_ssb', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    PSO4_ssb = 0e+0_fp
    ...
    ALLOCATE( PSO4_dustb( State_Grid%NX, State_Grid%NY, State_Grid%MaxChemLev, &
                         NDSTBIN ), STAT=RC )
    CALL GC_CheckVar( 'sulfate_mod.F90:PSO4_dustb', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    PSO4_dustb = 0e+0_fp
    ...
    id_SO4b  = Ind_('SO4b'     )
    id_SO4aq = Ind_('SO4aq'    )
    id_SO4aqb = Ind_('SO4aqb'    )
    id_SO4d1 = Ind_('SO4D1'    )
    id_SO4d1b = Ind_('SO4D1b'    )
    id_SO4d2 = Ind_('SO4D2'    )
    id_SO4d2b = Ind_('SO4D2b'    )
    id_SO4d3 = Ind_('SO4D3'    )
    id_SO4d3b = Ind_('SO4D3b'    )
    id_SO4d4 = Ind_('SO4D4'    )
    id_SO4d4b = Ind_('SO4D4b'    )
    id_SO4s  = Ind_('SO4s'     )
    id_SO4sb  = Ind_('SO4sb'     )
    ...
    DRYSO4sb  = Ind_('SO4sb', 'D')
    DRYSO4d1b = Ind_('SO4d1b','D')
    DRYSO4d2b = Ind_('SO4d2b','D')
    DRYSO4d3b = Ind_('SO4d3b','D')
    DRYSO4d4b = Ind_('SO4d4b','D')
    ...
    IF ( Input_Opt%LDSTUP ) THEN
       IF ( id_DAL1  < 0 .or. id_DAL2  < 0 .or. id_DAL3  < 0   .or. &
            ...
            id_SO4d2 < 0 .or. id_SO4d3 < 0 .or. id_SO4d4 < 0   .or. &
            id_SO4d1b< 0 .or. id_SO4d2b < 0 .or. id_SO4d4b < 0 ) THEN
...
!BOC
    ...
    IF ( ALLOCATED( PSO2b_DMS   ) ) DEALLOCATE( PSO2b_DMS  )
    IF ( ALLOCATED( PSO4_SO2   ) ) DEALLOCATE( PSO4_SO2   )
    IF ( ALLOCATED( PSO4b_SO2b ) ) DEALLOCATE( PSO4b_SO2b )
#ifdef APM
    ...
    IF ( ALLOCATED( PSO4_ssb   ) ) DEALLOCATE( PSO4_ssb   )
    IF ( ALLOCATED( PSO4_dust  ) ) DEALLOCATE( PSO4_dust  )
    IF ( ALLOCATED( PSO4_dustb ) ) DEALLOCATE( PSO4_dustb )
