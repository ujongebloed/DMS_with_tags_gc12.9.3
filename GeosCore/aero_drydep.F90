...
!
    ...
    INTEGER,  SAVE     :: H2SO4IDb ! UAJ
    INTEGER,  SAVE     :: id_H2SO4
    INTEGER,  SAVE     :: id_H2SO4b ! UAJ
    INTEGER,  SAVE     :: id_NK1
    ...
    IF ( FIRST ) THEN
       ...
       id_H2SO4 = Ind_('H2SO4')
       id_H2SO4b = Ind_('H2SO4b') ! UAJ
       id_NK1   = Ind_('NK1'  )

       ! Make sure species are defined
       IF ( id_H2SO4 < 0 .or. id_H2SO4b < 0 ) THEN ! UAJ
       ...
       DO BIN = 1, IBINS
          DO N   = 1, nDryDep
             ...
             IF ( BIN == 1 .and. &
                  State_Chm%Map_DryDep(N)==id_H2SO4b ) THEN  ! UAJ
                H2SO4IDb = N
                ! Debug
                !print *, 'DRYDEP Species:',N
             ENDIF
...
100       CONTINUE
    ...
    DO L = 1, State_Grid%MaxChemLev
       ...
       Y0b = Spc(I,J,L,id_H2SO4b)  ! UAJ
       RKTb = DEPSAV(I,J,H2SO4IDb) * State_Met%F_UNDER_PBLTOP(I,J,L) ! UAJ
       Yb = Y0b * EXP(-RKTb)  ! UAJ
       ...
       IF ( ND44 > 0 .AND. RKT > 0d0 ) THEN
          ...
          FLUXb = Y0b - Yb   ! UAJ
          FLUXb = FLUXb / (State_Chm%SpcData(id_H2SO4b)%Info%emMW_g * & ! UAJ
                 1.e-3_fp) / AREA_CM2 / DTCHEM * AVO

          AD44(I,J,H2SO4IDb,1) = AD44(I,J,H2SO4IDb,1) + FLUX
