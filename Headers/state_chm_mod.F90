...
!
  ...
  TYPE, PUBLIC :: ChmState
     ...
     REAL(fp),          POINTER :: SO2bAfterChem (:,:,:) !  after sulfate chem
     ...
     REAL(fp),          POINTER :: HSO3_AQb   (:,:,:  ) ! Cloud bisulfite[mol/l]
     REAL(fp),          POINTER :: SO3_AQ     (:,:,:  ) ! Cloud sulfite  [mol/l]
     REAL(fp),          POINTER :: SO3_AQb    (:,:,:  ) ! Cloud sulfite  [mol/l]
...
!
    ...
    State_Chm%SO2bAfterChem     => NULL() ! UAJ
    ...
    State_Chm%HSO3_AQb          => NULL()
    State_Chm%SO3_AQ            => NULL()
    State_Chm%SO3_AQb           => NULL()
    ...
    IF ( Input_Opt%ITS_A_FULLCHEM_SIM .or. Input_Opt%ITS_AN_AEROSOL_SIM ) THEN
       ...
       !------------------------------------------------------------------
       chmID = 'HSO3AQb'
       ALLOCATE( State_Chm%HSO3_AQb( IM, JM, LM ) , STAT=RC )
       CALL GC_CheckVar( 'State_Chm%HSO3_AQb', 0, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       State_Chm%HSO3_AQb = 0.0_fp
       CALL Register_ChmField( Input_Opt, chmID, State_Chm%HSO3_AQb,         &
                               State_Chm, RC                                )
       CALL GC_CheckVar( 'State_Chm%HSO3_AQb', 1, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       ...
       !------------------------------------------------------------------
       chmID = 'SO3AQb'
       ALLOCATE( State_Chm%SO3_AQb( IM, JM, LM ) , STAT=RC )
       CALL GC_CheckVar( 'State_Chm%SO3_AQb', 0, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       State_Chm%SO3_AQb = 0.0_fp
       CALL Register_ChmField( Input_Opt, chmID, State_Chm%SO3_AQb,          &
                               State_Chm, RC                                )
       CALL GC_CheckVar( 'State_Chm%SO3_AQb', 1, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
    ...
    !------------------------------------------------------------------
    chmID = 'SO2bAfterChem'
    ALLOCATE( State_Chm%SO2bAfterChem( IM, JM, LM ) , STAT=RC )
    CALL GC_CheckVar( 'State_Chm%SO2bAfterChem', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
    State_Chm%SO2bAfterChem = 0.0_fp
    CALL Register_ChmField( Input_Opt, chmID, State_Chm%SO2bAfterChem,     &  ! UAJ
                            State_Chm, RC                                )
    CALL GC_CheckVar( 'State_Chm%SO2bAfterChem', 1, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
...
!
    ...
    IF ( ASSOCIATED( State_Chm%SO2bAfterChem ) ) THEN
       DEALLOCATE( State_Chm%SO2bAfterChem, STAT=RC )
       CALL GC_CheckVar( 'State_Chm%SO2bAfterChem', 2, RC )  ! UAJ
       IF ( RC /= GC_SUCCESS ) RETURN
    ENDIF
    ...
    IF ( ASSOCIATED( State_Chm%HSO3_AQb ) ) THEN
       DEALLOCATE( State_Chm%HSO3_AQb, STAT=RC )
       CALL GC_CheckVar( 'State_Chm%HSO3_AQb', 3, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       State_Chm%HSO3_AQb => NULL()
    ENDIF
    ...
    IF ( ASSOCIATED( State_Chm%SO3_AQb ) ) THEN
       DEALLOCATE( State_Chm%SO3_AQb, STAT=RC )
       CALL GC_CheckVar( 'State_Chm%SO3_AQb', 3, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       State_Chm%SO3_AQb => NULL()
    ENDIF
    ...
    !      IF ( isRank  ) Rank  = 3
     ...
     !     IF ( isDesc  ) Desc  = 'Wet aerosol radius for sulfate  on' &
      !                            // 'surface of seasalt aerosol'
       ...
       CASE ( 'HSO3AQB' )
          IF ( isDesc  ) Desc  = 'Cloud bisulfiteb concentration'
          IF ( isUnits ) Units = 'mol L-1'
          IF ( isRank  ) Rank  =  3
       ...
       CASE ( 'SO3AQB' )
          IF ( isDesc  ) Desc  = 'Cloud sulfiteb concentration'
          IF ( isUnits ) Units = 'mol L-1'
          IF ( isRank  ) Rank  =  3
       ...
       CASE ( 'SO2BAFTERCHEM' )
          IF ( isDesc  ) Desc  = 'SO2b after sulfate chemistry' ! UAJ
          IF ( isUnits ) Units = 'mol mol-1'
          IF ( isRank  ) Rank  =  3
...
!
    ...
    N = Ind_('DMS')
    IF ( N > 0 ) THEN
       State_Chm%HetInfo%DMS%mId  = N
       State_Chm%HetInfo%DMS%MW_g = State_Chm%SpcData(N)%Info%MW_g
       State_Chm%HetInfo%DMS%K0   = State_Chm%SpcData(N)%Info%Henry_K0
       State_Chm%HetInfo%DMS%CR   = State_Chm%SpcData(N)%Info%Henry_CR
       State_Chm%HetInfo%DMS%TK   = 298.15_fp
    ELSE
       errMsg = &
        'DMS is undefined, but is needed for heterogeneous chemistry!'
       CALL GC_Error( errmsg, RC, thisLoc )
       RETURN
    ENDIF
    ...
    N = Ind_('DMSO')
    IF ( N > 0 ) THEN
       State_Chm%HetInfo%DMSO%mId  = N
       State_Chm%HetInfo%DMSO%MW_g = State_Chm%SpcData(N)%Info%MW_g
       State_Chm%HetInfo%DMSO%K0   = State_Chm%SpcData(N)%Info%Henry_K0
       State_Chm%HetInfo%DMSO%CR   = State_Chm%SpcData(N)%Info%Henry_CR
       State_Chm%HetInfo%DMSO%TK   = 298.15_fp
    ELSE
       errMsg = &
        'DMSO is undefined, but is needed for heterogeneous chemistry!'
       CALL GC_Error( errmsg, RC, thisLoc )
       RETURN
    ENDIF
    ...
    N = Ind_('MSIA')
    IF ( N > 0 ) THEN
       State_Chm%HetInfo%MSIA%mId  = N
       State_Chm%HetInfo%MSIA%MW_g = State_Chm%SpcData(N)%Info%MW_g
       State_Chm%HetInfo%MSIA%K0   = State_Chm%SpcData(N)%Info%Henry_K0
       State_Chm%HetInfo%MSIA%CR   = State_Chm%SpcData(N)%Info%Henry_CR
       State_Chm%HetInfo%MSIA%TK   = 298.15_fp
    ELSE
       errMsg = &
        'MSIA is undefined, but is needed for heterogeneous chemistry!'
       CALL GC_Error( errmsg, RC, thisLoc )
       RETURN
    ENDIF
    ...
    N = Ind_('MSA')
    IF ( N > 0 ) THEN
       State_Chm%HetInfo%MSA%mId  = N
       State_Chm%HetInfo%MSA%MW_g = State_Chm%SpcData(N)%Info%MW_g
       State_Chm%HetInfo%MSA%K0   = State_Chm%SpcData(N)%Info%Henry_K0
       State_Chm%HetInfo%MSA%CR   = State_Chm%SpcData(N)%Info%Henry_CR
       State_Chm%HetInfo%MSA%TK   = 298.15_fp
    ELSE
       errMsg = &
        'MSA is undefined, but is needed for heterogeneous chemistry!'
       CALL GC_Error( errmsg, RC, thisLoc )
       RETURN
    ENDIF
    ...
    N = Ind_('HPMTF')
      IF ( N > 0 ) THEN
         State_Chm%HetInfo%HPMTF%mId  = N
         State_Chm%HetInfo%HPMTF%MW_g = State_Chm%SpcData(N)%Info%MW_g
         State_Chm%HetInfo%HPMTF%K0   = State_Chm%SpcData(N)%Info%Henry_K0
         State_Chm%HetInfo%HPMTF%CR   = State_Chm%SpcData(N)%Info%Henry_CR
         State_Chm%HetInfo%HPMTF%TK   = 298.15_fp
      ELSE
         errMsg = &
          'HPMTF is undefined, but is needed for heterogeneous chemistry!'
       CALL GC_Error( errmsg, RC, thisLoc )
       RETURN
    ENDIF
    ...
    N = Ind_('SO4b')
    IF ( N > 0 ) THEN
       State_Chm%HetInfo%SO4b%mId  = N
       State_Chm%HetInfo%SO4b%MW_g = State_Chm%SpcData(N)%Info%MW_g
       State_Chm%HetInfo%SO4b%K0   = State_Chm%SpcData(N)%Info%Henry_K0
       State_Chm%HetInfo%SO4b%CR   = State_Chm%SpcData(N)%Info%Henry_CR
       State_Chm%HetInfo%SO4b%TK   = 298.15_fp
    ELSE
       errMsg = &
        'SO4b is undefined, but is needed for heterogeneous chemistry!'
       CALL GC_Error( errmsg, RC, thisLoc )
       RETURN
    ENDIF
